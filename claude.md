...
- [x] Mike's Tavern Issues sheet, batch A (2026-07-24): SRD/Bestiary hidden behind lib/site-config.ts flags (nothing deleted — flip SHOW_SRD/SHOW_BESTIARY to restore; nav/home link to sagabornd100.com meanwhile); Dodge → Combat category (supersedes errata #7); sheet shows canonical characteristic rolls (STR ROLL ×5 etc., pools retired) + speed/horror/mana filled; Normal Mode "(Starter Characters)"; scroll-to-top on step change; sticky lifepath Confirm; brighter skill values
- [ ] Mike's Tavern Issues sheet, batch B (queued): talents at creation (CRITICAL), equipment choose/buy (CRITICAL), editable-at-table sheet — wounds/horror/mana/checkboxes (CRITICAL), sheet matches printed sheets, archetype-first quick generator, character portraits, SagaBorn Tavern rebrand + logos (parked for Mike's design screenshot)
- [x] Mike's punch list round 2 (2026-07-25): profession skill-choice resolution — "Melee Weapon Skill (Any)" chooser on the skills step (13 professions), fixed-specialty grants (Knowledge (Arcana) etc.) resolve to their base rows (fixes Mystic's missing Knowledge), Knowledge capped at 40 (003 §Skill Specialties — splits into Expertise above), Parry removed as a skill (commented in skills.ts; rating = weapon skill), category bonus fixed to ⌈char/2⌉ in the builder (was 13/16 thresholds), name rebranded to SagaBorn Tavern (text only — logos pending), CharacterList no longer collapses during background refetches
- [ ] Mike items needing clarification: "rolling dice makes noise" and "level-up popup says success or fail" — no audio code or such popup exists in the app; ask what he's seeing
- [ ] Docs re-sync: 001/003/004/010 edited after the 7-14 pull (Dodge fix landed in 003; professions-chart move NOT yet visible in 002) — re-pull → re-encode → validators
